...
subtitle: "Single CpG test with resilience index ---- MCI samples"
author: 
...
dir_results_res <- file.path(dir_results, "resilience_mci")
...
```{r, eval = F}
pheno <- pheno %>% filter(dis_global == 3)
...
```{r}
results_all <- read_csv(
  file.path(dir_results_res, "../resilience/Resilience_rlm_model_results_bacon.csv")
)
results_mci <- results_bacon$data.with.inflation
```

```{r}
results_all_sig <- results_all %>%
  filter(pValue.bacon < 1e-05) %>%
  arrange(pValue.bacon)
```

```{r}
results_mci_filtered <- results_mci[match(results_all_sig$cpg, results_mci$cpg),]
write_csv(
  results_mci_filtered,
  file.path(dir_results_res, "Resilience_sig_all_results.csv")
)
```
