...
subtitle: "Run line by line preprocessing of DNA methylation data"
author:
...
probe_dir <- file.path(save_dir, "data", "Epigenetic_clocksOLD")
...
qc_fix_summary_beta <- function(
  ...
  to_keep <- colnames(assay_data) %in% colnames(fixed_data)
...
get_probe_counts <- function(df_probes, label = "") {
  counts <- c(table(df_probes$Source))
  counts$Total <- sum(counts)
  counts <- data.frame(c(step = label, counts))
...
df_probes <- read.csv(file.path(probe_dir, "datMiniAnnotation4_fixed (1).csv")) %>%
  dplyr::mutate(
    Source = ifelse(
      stringr::str_starts(.data$Source, "Control"),
      "Control",
      .data$Source
    )
  ) %>%
...
qc_epigenetic_df <- get_probe_counts(df_probes, label = "All Probes")
```
...
df_probes <- df_probes %>%
  dplyr::filter(.data$Name %in% probes)

qc_epigenetic_df <- rbind(
  qc_epigenetic_df,
  get_probe_counts(df_probes, label = "Present in EPICv2")
)
```
...
# record progress
df_probes <- df_probes %>%
  dplyr::filter(.data$Name %in% qc_get_v2_names(row.names(assay_data)))

qc_epigenetic_df <- rbind(
  qc_epigenetic_df,
  get_probe_counts(df_probes, label = "Passed Detection P")
)
...
# record progress
df_probes <- df_probes %>%
  dplyr::filter(.data$Name %in% qc_get_v2_names(row.names(dnam_data)))

qc_epigenetic_df <- rbind(
  qc_epigenetic_df,
  get_probe_counts(df_probes, label = "Filted by SNP proximity and cross-hybridization")
)
...
qc_epigenetic_df <- rbind(
  qc_epigenetic_df,
  get_probe_counts(df_probes, label = "Filtered and Imputed Missing")
)

assay_dimensions <- get_data_dimensions(dnam_data)
qc_summary_df[6, "step"] <- "Filtered and imputed missing values"
...
qc_summary_df[6, "R_package"] <- "impute::impute.knn(maxp = 10000)"
...
qc_summary_df[7, "step"] <- "Normalized Data"
qc_summary_df[7, "samples"] <- assay_dimensions$samples
qc_summary_df[7, "probes"] <- assay_dimensions$probes
qc_summary_df[7, "unique_probes"] <- assay_dimensions$unique_probes
qc_summary_df[7, "R_package"] <- paste0(
...
assay_xy <- assay_xy[,colnames(assay_data)]

# record progress
df_probes <- df_probes %>%
  dplyr::filter(.data$Name %in% qc_get_v2_names(c(row.names(assay_data), row.names(assay_xy))))

qc_epigenetic_df <- rbind(
  qc_epigenetic_df,
  get_probe_counts(df_probes, label = "Filtered and Imputed Missing after Normalization")
)

assay_dimensions <- get_data_dimensions(rbind(assay_data, assay_xy))
qc_summary_df[8, "step"] <- "Filtered and imputed missing values"
qc_summary_df[8, "samples"] <- assay_dimensions$samples
qc_summary_df[8, "probes"] <- assay_dimensions$probes
qc_summary_df[8, "unique_probes"] <- assay_dimensions$unique_probes
qc_summary_df[8, "R_package"] <- "impute::impute.knn(maxp = 10000)"
...
# record progress
qc_summary_df[10, "step"] <- "Predict cell type proportions"
...
qc_summary_df[10, "R_package"] <- "EpiDISH::epidish(ref.m = EpiDISH::centDHSbloodDMC.m, method = 'RPC')"
```
...
# Record Progress
qc_summary_df[11, "step"] <- "predict sample sex"
qc_summary_df[11, "samples"] <- ncol(dnam_data)
qc_summary_df[11, "probes"] <- nrow(dnam_data)
qc_summary_df[11, "unique_probes"] <- nrow(dnam_data)
qc_summary_df[11, "R_package"] <- package <- "wateRmelon::estimateSex"
```
...
qc_summary_df[12, "unique_probes"] <- nrow(dnam_data)
...
## Show Epigenetic Probes Table

```{r show_df_1, echo=FALSE}
qc_epigenetic_df %>%
  DT::datatable(
    filter = 'top',
    style = "bootstrap",
    ...
    rownames = FALSE,
    caption = "Epigenetic Probe Counts"
  )
```
...
write.csv(
  qc_epigenetic_df,
  file = file.path(save_dir, "analysis_results", "epigenetic_probes.csv"),
  row.names = FALSE
)
ggplot2::ggsave(
