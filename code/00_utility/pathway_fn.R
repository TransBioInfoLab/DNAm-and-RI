...
# ===================================================================================================
methylGSA_wrapper <- function(cpg.pval, array, method = "GSEA", GS, add_anno_for_sig = T, anno_df = NULL,
  ...
  if(add_anno_for_sig) {
    if(array == "EPICv2") {
      CpGs <- anno_df %>% 
        mutate(Name = cpg) %>%
        dplyr::select(Name, UCSC_RefGene_Name, UCSC_RefGene_Group)
    } else {
  ...
  results <- plyr::llply(
    ...
    .fun = function(gs.type) {
      if(use_msigdbr) {
        gs.type.split <- str_split(gs.type,":")[[1]]
        category <- gs.type.split[1]
        subc <- NULL
        if(length(gs.type.split) == 2) {
          subc <- toupper(gs.type.split[2])
        }

        pathway_df <- msigdbr(species = "Homo sapiens", category = category, subcategory = subc)
        pathway_df <- data.frame(pathway_df)
        pathway <- unique(pathway_df$gs_name)
        GS.list <- plyr::llply(
          pathway,
          .fun = function(p) {
            pathway_df[pathway_df$gs_name %in% p,"gene_symbol"] 
          }
        )

        names(GS.list) <- pathway  
        gs.type <- "GO" # The GS.type parameter in the function must match one of "GO", "KEGG", or "Reactome", even when inputting a custom pathway list.
